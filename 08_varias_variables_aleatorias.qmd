...
$$
\tilde{p}(k) = \lim_{dt\rightarrow\infty} \left(\lambda dt e^{-ik} + (1 - \lambda dt)\right)^{T/dt} = \exp\left[\lambda (e^{-ik} - 1)\right]\,.
$$
De aquí tenemos que
$$
...
Estudiemos el caso de $n$ variables aleatorias $x_1,\dots,x_m$ que organizamos en un vector $\mathbf{x}$. Definimos la función de distribución conjunta de probabilidad para las varias variables $\rho(\mathbf{x})$, tal que
$$
